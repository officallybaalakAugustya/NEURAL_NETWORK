...
class ReplayMemory:
    ...
    def sample(self, batch_size: int = BATCH_SIZE) -> list:
        if len(self.buffer) < batch_size:
            raise ValueError(f"Cannot sample {batch_size} from buffer.")
...
class Trainer:
    ...
    def select_action(self, state: np.ndarray) -> int:
        ...
        q_values = self.model.forward(state_2d)
    ...
    def _train_on_batch(self, experiences: list) -> float:
        ...
        for state, action, reward, next_state, done in experiences:
            ...
            current_q = self.model.forward(state_2d)
            target_q = np.copy(current_q)
            
            # THE BUG FIX: target_q[0][action] specifies Row 0, Column 'action'
            if done:
                target_q[0][action] = reward
            else:
                next_q = self.model.forward(next_state_2d)
                target_q[0][action] = reward + GAMMA * float(np.max(next_q))
    ...
    def run_episode(self) -> tuple[float, float]:
        ...
        self.decay_epsilon()
        avg_loss: float = float(np.mean(episode_losses)) if episode_losses else 0.0
        return episode_reward, avg_loss

    def train(self) -> None:
        logger.info("=" * 60)
        logger.info("Training started (Deep Network + Adam Optimizer)")
        logger.info("Episodes: %d | Batch: %d | Memory: %d", EPISODES, BATCH_SIZE, MEMORY_SIZE)
        logger.info("=" * 60)   

        record_score : float = -9999.0 
        ...
        try:
            for episode in range(1, EPISODES + 1):
                ...
                logger.info(
                    ...
                    episode, EPISODES, episode_reward, avg_loss, self.epsilon,
                )
          
        except KeyboardInterrupt:
            logger.warning("Training interrupted by user at episode %d.", episode)
        ...
        logger.info("=" * 60)
        logger.info("Training complete. Files generated successfully.")
        logger.info("=" * 60)
    ...
    def _save_metrics(self) -> None:
        try:
            reward_df = pd.DataFrame({"episode": range(1, len(self.reward_history) + 1), "total_reward": self.reward_history})
            reward_df.to_csv("reward_history.csv", index=False)
            
            loss_df = pd.DataFrame({"episode": range(1, len(self.loss_history) + 1), "average_loss": self.loss_history})
